...
# 受理/拒绝分类依据 ACCEPTED_BATCH_STATUS（定义随 KNOWN_BATCH_STATUS 收敛在
# app/clients/wedap.py）：其余 5 值为业务拒绝——deliver_task 抛 WedapBatchRejected 进
# dispatch 退避重试，不再被误记 DELIVERED（旧 silent-failure：拒绝批永远等不到 result）。
